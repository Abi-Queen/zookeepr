...
function filterByQuery(query, zookeepers) {
    ...
    if (query.age) {
        filteredResults = filteredResults.filter(
            //since our form data will be coming in as strings, and our JSON is storing age as number, we convert query string to number to perform comparison.
...
function validateZookeeper(zookeeper) {
    if (!zookeeper.age || typeof zookeeper.name !== "string") {
        return false;
    }
    if (!zookeeper.age || typeof zookeeper.age !== "string"
    ) {
        return false;
    }
    return true;
